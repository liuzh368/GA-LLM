...
def simplify_poi_category(text):
    """
    Simplify the POI category name by extracting the 'name' field from JSON-like strings,
    handling cases where 'name' contains special characters like double quotes.
    """
    # 修改正则表达式以适应name字段包含双引号的情况
    pattern = r"'name':\s*['\"]([^'\"]+)['\"]"
    match = re.search(pattern, text)
    # 如果匹配成功，返回 name 字段的内容；否则返回原始文本
...
def main():
    ...
    # Read the data
    train_data = pd.read_csv(f'{path}train_sample_min.csv')
    test_data = pd.read_csv(f'{path}test_sample_with_traj_min.csv')
    train_data['PoiCategoryName'] = train_data['PoiCategoryName'].apply(simplify_poi_category)

    # Save the modified DataFrame to a new CSV file
    train_data.to_csv(f'{path}train_sample_min.csv', index=False)
    test_data['PoiCategoryName'] = test_data['PoiCategoryName'].apply(simplify_poi_category)

    # Save the modified DataFrame to a new CSV file
    test_data.to_csv(f'{path}test_sample_min.csv', index=False)
    ...
    with open(f'{path}train_kq_pairs_min.json', 'w') as json_file:
    ...
    with open(f'{path}test_kq_pairs_min.json', 'w') as json_file:
