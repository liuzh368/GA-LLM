    ...
    for user in tqdm(main_data['UserId'].unique()):
        ...
        for traj_id in user_data['pseudo_session_trajectory_id'].unique():
            ...
            if num_traj > 200:
                ...
                for i, row in user_trajectory_data.iloc[:-1].iterrows():
                    question_parts.append(
                        ...
                        f", located at coordinates ({row['Latitude']}, {row['Longitude']}) with the coordinates token <GPS {row['PoiId']}>."
                    )
            else:
                # 1. 处理相似轨迹
            ...
            question += f" Based on this data, predict the location user {user} will visit next. " \
                        ...
                        f"with the coordinates token <GPS {row['PoiId']}>. What are its corresponding latitude and longitude coordinates?" \


            # 生成答案
            answer = f"<answer>: User {user} will visit a POI at coordinates ({next_latitude}, {next_longitude})."
...
def main():
    ...
    with open(f'{path}train_qa_pairs_kqt_200items_{args.dataset_name}_projector_gps_without_sim_stage1.json', 'w') as json_file:
    ...
    # 保存测试数据为 TXT 文件
    with open(f'{path}test_qa_pairs_kqt_200items_{args.dataset_name}_projector_gps_without_sim_stage1.txt', 'w') as txt_file:
