            ...
            if num_traj > 200:
                ...
                for i, row in user_trajectory_data.iloc[:-1].iterrows():
                    question_parts.append(
                        ...
                        f" The location is at coordinates ({row['Latitude']}, {row['Longitude']}) with token <GPS {row['PoiId']}>."
...
def main():
    ...
    with open(f'{path}train_qa_pairs_kqt_200items_{args.dataset_name}_llm_without_sim_gps_poi_new3.json', 'w') as json_file:
    ...
    # 保存测试数据为 TXT 文件
    with open(f'{path}test_qa_pairs_kqt_200items_{args.dataset_name}_llm_without_sim_gps_poi_new3.txt', 'w') as txt_file:
