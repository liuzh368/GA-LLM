            ...
            if num_traj > 200:
                ...
                for i, row in user_trajectory_data.iloc[:-1].iterrows():
                    question_parts.append(
                        ...
                        f"located at coordinates ({row['Latitude']:.6f}, {row['Longitude']:.6f}) with the coordinates token <GPS {row['PoiId']}>. "
                        f"It is a {row['PoiCategoryName']} with category ID {row['PoiCategoryId']}."
                    )
            else:
                # 1. 处理相似轨迹（最不重要，放最前）
                num_similar_traj = 0
...
def main():
    ...
    with open(f'{path}train_qa_pairs_kqt_200items_{args.dataset_name}_llm_without_sim_gps.json', 'w') as json_file:
    ...
    # 保存测试数据为 TXT 文件
    with open(f'{path}test_qa_pairs_kqt_200items_{args.dataset_name}_llm_without_sim_gps.txt', 'w') as txt_file:
