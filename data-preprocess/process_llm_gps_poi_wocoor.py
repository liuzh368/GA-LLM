            ...
            if num_traj > 200:
                ...
                for i, row in user_trajectory_data.iloc[:-1].iterrows():
                    question_parts.append(
                        ...
                        f" The location is at <GPS {row['PoiId']}>."
...
def main():
    ...
    with open(f'{path}train_qa_pairs_kqt_200items_{args.dataset_name}_llm_without_sim_gps_poi_wocoor.json', 'w') as json_file:
    ...
    # 保存测试数据为 TXT 文件
    with open(f'{path}test_qa_pairs_kqt_200items_{args.dataset_name}_llm_without_sim_gps_poi_wocoor.txt', 'w') as txt_file:
