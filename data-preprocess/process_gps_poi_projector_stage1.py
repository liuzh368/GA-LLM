...
def generate_qa_pairs(main_data, kqt=None, historical_data=None, args=None, use_sim=False):
    ...
    for user in tqdm(main_data['UserId'].unique()):
        ...
        for traj_id in user_data['pseudo_session_trajectory_id'].unique():
            ...
            if num_traj <= 600:
                ...
                if historical_data is not None:
                    ...
                    early_historical_data = user_data[
                        (user_data['UTCTimeOffset'] < start_time_of_current_traj)
                    ].tail(600)
            ...
            # 开始拼接问题部分，凸显重要性顺序：1. 相似轨迹 > 2. 早期历史轨迹 > 3. 当前轨迹
            ...
            if num_traj > 200:
                ...
                for i, row in user_trajectory_data.iloc[:-1].iterrows():
                    question_parts.append(
                        f"At {row['UTCTimeOffset']}, user {user} visited POI id {row['PoiId']} (token is <POI {row['PoiId']}>)"
                        f", coordinates: ({row['Latitude']}, {row['Longitude']}) (token is <GPS {row['PoiId']}>)."
                    )
            else:
                # 1. 处理相似轨迹
                if use_sim and not similar_historical_data.empty:
                    ...
                    for _, row in similar_historical_data.iterrows():
                        question_parts.append(
                            f"At {row['UTCTimeOffset']}, user {row['UserId']} visited POI id {row['PoiId']} (token is <POI {row['PoiId']}>)"
                            f", coordinates: ({row['Latitude']}, {row['Longitude']}) (token is <GPS {row['PoiId']}>)."
                        )
            ...
            next_latitude = user_trajectory_data.iloc[-1]['Latitude']  # 最后一个 POI 的纬度
            next_longitude = user_trajectory_data.iloc[-1]['Longitude']  # 最后一个 POI 的经度
            question += f" Based on this data, predict the location user {user} will visit next. " \
                        ...
                        f"(token is <POI {next_poi_id}>) with coordinates (token is <GPS {next_poi_id}>). " \
                        f"Which POI id will user {user} visit? Provide both the POI id and the corresponding coordinates. " \
                        f"Note that POI id is an integer in the range from 0 to {value}."

            # 生成答案
            answer = f"<answer>: User {user} will visit POI id {next_poi_id} at coordinates ({next_latitude}, {next_longitude})."
...
def main():
    ...
    with open(f'{path}train_qa_pairs_kqt_200items_{args.dataset_name}_projector_gps_poi_without_sim_new.json', 'w') as json_file:
    ...
    # 保存测试数据为 TXT 文件
    with open(f'{path}test_qa_pairs_kqt_200items_{args.dataset_name}_projector_gps_poi_without_sim_new.txt', 'w') as txt_file:
