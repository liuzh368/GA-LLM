            ...
            if num_traj > 200:
                ...
                for i, row in user_trajectory_data.iloc[:-1].iterrows():
                    question_parts.append(
                        f"At {row['UTCTimeOffset']}, user {user} visited POI id {row['PoiId']}, the token for this POI is <POI {row['PoiId']}>."
                    )
            else:
                # 1. 处理相似轨迹（最不重要，放最前）
                num_similar_traj = 0
                if use_sim and not similar_historical_data.empty:
                    ...
                    for _, row in similar_historical_data.iterrows():
                        question_parts.append(
                            f"At {row['UTCTimeOffset']}, user {row['UserId']} visited POI id {row['PoiId']}, the token for this POI is <POI {row['PoiId']}>."
                        )
...
def main():
    ...
    with open(f'{path}train_qa_pairs_kqt_200items_{args.dataset_name}_llm_wosim_cat.json', 'w') as json_file:
    ...
    # 保存测试数据为 TXT 文件
    with open(f'{path}test_qa_pairs_kqt_200items_{args.dataset_name}_llm_wosim_cat.txt', 'w') as txt_file:
