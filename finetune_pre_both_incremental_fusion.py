# finetune_pre_both_incremental_fusion.py
...
# ====== 将这里的导入由 geo_model.geo_encoder 改为 fusion_geo_encoder ======
# from geo_model.geo_encoder import GeoEncoder
from geo_model.fusion_geo_encoder import FusionGeoEncoder4096
...
class ModelArguments:
    ...
    geo_encoder_path: Optional[str] = field(
        default=None, metadata={"help": "Path to the pretrained FusionGeoEncoder (optional)."}
    )
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    lora_r_geo_encoder: int = field(
        default=8,
        metadata={"help": "Rank for the LoRA low-rank matrix approximation for the FusionGeoEncoder."}
    )
    lora_alpha_geo_encoder: float = field(
        default=32,
        metadata={"help": "Alpha scaling factor for combining LoRA weights for the FusionGeoEncoder."}
    )
...
def embedding_hook(module, inputs, output):
    ...
    if gps_token_start_id is not None and max_token_id is not None:
        ...
        if gps_token_ids.size(0) > 0:
            ...
            # Use FusionGeoEncoder to generate embeddings
            geo_embeddings = []
            for lat, lng in gps_coordinates:
                embedding = module.geo_encoder(lat, lng)  # 注意：FusionGeoEncoder的forward接口
...
class POITrainer(Trainer):
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        try:
            ...
            print(f"Merged FusionGeoEncoder saved to {geo_encoder_save_path}")
        except AttributeError as e:
...
def save_combined_linear_mapping_layer(model, output_path):
    """Merge LoRA weights into linear_mapping_layer and save."""
    with torch.no_grad():
        def get_linear_mapping_layer(model):
            if hasattr(model, 'linear_mapping_layer'):
                print("Accessed linear_mapping_layer at model.linear_mapping_layer")
...
def merge_lora_weights_geo_encoder(geo_encoder):
    """Merge LoRA weights into base FusionGeoEncoder."""
...
def train():
    ...
    print(f"  GeoEncoder (Fusion) Path: {model_args.geo_encoder_path}")
    ...
    print(f"  LoRA r (Main Model): {training_args.lora_r_main_model}, Alpha: {training_args.lora_alpha_main_model}")
    print(f"  LoRA r (FusionGeoEncoder): {training_args.lora_r_geo_encoder}, Alpha: {training_args.lora_alpha_geo_encoder}")
    ...
    # If a PEFT directory is provided, load LoRA parameters
    ...
    poi_embedding_layer = nn.Embedding(num_embeddings=num_poi_tokens, embedding_dim=128)  # original 128-dim
    ...
    # ========== 在这里改成 FusionGeoEncoder4096 ==========
    # 你可以根据自己的需求，自行修改 gps_embed_dim / final_dim 等
    geo_encoder = FusionGeoEncoder4096(
        gps_embed_dim=128,          # n-gram/transformer分支的embed大小
        quadkey_length=25,         # QuadKey长度
        n=6,                       # n-gram的n
        vocab_size=4096,           # n-gram embedding中Embedding大小(或可自行设置)
        fourier_output_dim=128,    # 可学习傅里叶分支输出大小
        final_dim=model.config.hidden_size,  # 最终映射到与 LLM hidden_size 一致
        dropout=0.1
    )

    if model_args.geo_encoder_path:
        # 加载预训练的FusionGeoEncoder权重（如果有）
        ...
        print(f"Loaded pretrained FusionGeoEncoder from {model_args.geo_encoder_path}")
    ...
    def set_geo_encoder(model, geo_encoder):
        if hasattr(model, 'base_model') and hasattr(model.base_model.model, 'model'):
            ...
            print("Set fusion_geo_encoder in model.base_model.model.model")
        elif hasattr(model, 'base_model') and hasattr(model.base_model.model, 'geo_encoder'):
            model.base_model.model.geo_encoder = geo_encoder
            print("Set fusion_geo_encoder in model.base_model.model")
        elif hasattr(model, 'base_model') and hasattr(model.base_model, 'geo_encoder'):
            model.base_model.geo_encoder = geo_encoder
            print("Set fusion_geo_encoder in model.base_model")
        elif hasattr(model, 'model') and hasattr(model.model, 'geo_encoder'):
            model.model.geo_encoder = geo_encoder
            print("Set fusion_geo_encoder in model.model")
        elif hasattr(model, 'geo_encoder'):
            model.geo_encoder = geo_encoder
            print("Set fusion_geo_encoder in model")
        else:
            for name, module in model.named_modules():
                if hasattr(module, 'geo_encoder'):
                    print(f"Found geo_encoder at: {name}.geo_encoder => replaced with FusionGeoEncoder")
    ...
    try:
        ...
        if geo_enc is not None:
            print("fusion_geo_encoder attached successfully.")
        else:
            print("fusion_geo_encoder not found in the model.")
    except AttributeError:
        print("fusion_geo_encoder not found in the model.")
    ...
    # Enable requires_grad for linear_mapping_layer and fusion_geo_encoder
    ...
    print("\nModules in fusion_geo_encoder:")
    for name, module in geo_encoder.named_modules():
        print(f" - {name}: {module.__class__.__name__}")

    # 针对 FusionGeoEncoder 做 LoRA
    # 注意 FusionGeoEncoder4096 里包含:
    #   self.gps_embed_model  -> GPSEmbeddings
    #   self.gps_encoder     -> GPSEncoder (内部 transformer_encoder)
    #   self.fourier_encoder -> GPSPositionalEncoder
    #   self.final_linear
    # 这里示例仅对 embedding + transformer 相关做 LoRA，如有需要可自行补充
    lora_config_geo = LoraConfig(
        r=training_args.lora_r_geo_encoder,
        lora_alpha=training_args.lora_alpha_geo_encoder,
        target_modules=[
            # 重点对 gps_embed_model 和 gps_encoder 做 LoRA
    ...
    # Merge and save FusionGeoEncoder
    geo_encoder = get_geo_encoder(model)
    if isinstance(geo_encoder, PeftModel):
        print("Unwrapping PeftModel to get base geo_encoder for saving")
        base_geo_encoder = geo_encoder.get_base_model()
    ...
    torch.save(base_geo_encoder.state_dict(), geo_encoder_save_path)
    print(f"Final merged FusionGeoEncoder saved to {geo_encoder_save_path}")
