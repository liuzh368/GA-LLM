...
def apply_embedding_hook(model, poi_embedding_layer, poi_token_start_id, gps_mapping, gps_id_list, gps_token_start_id, geo_encoder, max_token_id):
    ...
    if poi_embedding_layer is not None:
        ...
        # 根据模型结构，调整 linear_mapping_layer 的访问路径
        # 定义一个函数来获取 linear_mapping_layer
        def get_linear_mapping_layer(model):
            ...
            elif hasattr(model, 'model') and hasattr(model.model.embed_tokens, 'linear_mapping_layer'):
                print("Accessed linear_mapping_layer at model.model.embed_tokens.linear_mapping_layer")
                return model.model.embed_tokens.linear_mapping_layer
            elif hasattr(model, 'base_model') and hasattr(model.base_model.model.model.embed_tokens, 'linear_mapping_layer'):
                print("Accessed linear_mapping_layer at model.base_model.model.model.embed_tokens.linear_mapping_layer")
                return model.base_model.model.model.embed_tokens.linear_mapping_layer
            else:
                raise AttributeError("linear_mapping_layer not found in the model.")
        # 使用该函数获取 linear_mapping_layer
...
class POITrainer(Trainer):
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        # 保存 merged linear_mapping_layer
        try:
        ...
        # 保存 merged GeoEncoder
        try:
            geo_encoder = get_geo_encoder(model)
            # Merge LoRA weights into GeoEncoder
            merge_lora_weights_geo_encoder(geo_encoder)
            # Save the merged GeoEncoder
            geo_encoder_save_path = os.path.join(output_dir, "geo_encoder_merged.pth")
            torch.save(geo_encoder.state_dict(), geo_encoder_save_path)
...
def train():
    ...
    linear_mapping_layer = nn.Linear(128, model.config.hidden_size).to(device)
    ...
    # 将 linear_mapping_layer 注册为模型的子模块
    # 将其添加到模型的 embedding 层
    model.get_input_embeddings().linear_mapping_layer = linear_mapping_layer
    ...
    model.geo_encoder = geo_encoder.to(device)
    ...
    # 应用 LoRA 之前，打印模型结构
    print("\nModel structure before applying LoRA:")
    for name, module in model.named_modules():
        if 'linear_mapping_layer' in name:
            print(f"Found linear_mapping_layer at: {name}")
    ...
    for name, module in model.geo_encoder.named_modules():
        print(f" - {name}: {module.__class__.__name__}")

    # Adjust target_modules for geo_encoder based on actual module names
    # 调整 geo_encoder 的 target_modules，使用通配符
