...
def generate_qa_pairs(main_data, kqt=None, historical_data=None, args=None):
    ...
    for user in tqdm(main_data['UserId'].unique()):
        ...
        for traj_id in user_data['pseudo_session_trajectory_id'].unique():
            ...
            #             300 - num_traj)
            user_historical_data = pd.DataFrame()
            ...
            if str(traj_id) in kqt.keys():
                ...
                if historical_data is not None:
                    ...
                    if matched_historical_data.shape[0] >= (200 - num_traj):
                        user_historical_data = pd.concat(
                            [user_historical_data, matched_historical_data.tail(200 - num_traj)])
                    else:
                        user_historical_data = pd.concat([user_historical_data, matched_historical_data])
                else:
                    ...
                    # 从匹配的历史数据中取出200 - num_traj条最新的历史数据
                    user_historical_data = pd.concat([user_historical_data, matched_main_data.tail(200 - num_traj)])


            # 这里处理 historical_data 和 user_data 的逻辑，直接添加到 user_historical_data 后面
            if historical_data is not None:
                additional_historical_data = historical_data[
                    (historical_data['UserId'] == user) &
                    (historical_data['UTCTimeOffsetEpoch'] < start_time_of_current_traj)
                    ].tail(600 - num_traj)
            else:
                additional_historical_data = user_data[
                    (user_data['UTCTimeOffsetEpoch'] < start_time_of_current_traj)
                ].tail(600 - num_traj)

            # 将新的 additional_historical_data 添加到 user_historical_data
            user_historical_data = pd.concat([user_historical_data, additional_historical_data])
...
def main():
    ...
    with open(f'{path}train_qa_pairs_kqt_200items_ca_merged_geo.json', 'w') as json_file:
        json.dump(qa_dict_train, json_file)


    # Save the test QA pairs in TXT format
    with open(f'{path}test_qa_pairs_kqt_200items_ca_merged_geo.txt', 'w') as txt_file:
