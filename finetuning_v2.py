...
def initialize_poi_embeddings(model, tokenizer, poi_embeddings, poi_mapping_layer):
    """Initialize the POI tokens embeddings by mapping POI embeddings to LLM's embedding space."""
    ...
    poi_embeddings_mapped = poi_mapping_layer(poi_embeddings)
    ...
    for idx, poi_id in enumerate(poi_token_ids):
        embedding_weight[poi_id] = poi_embeddings_mapped[idx]
...
def train():
    ...
    # 定义POI映射层，将128维的POI embedding映射到4096维
    poi_mapping_layer = nn.Linear(poi_embeddings.shape[1], model.config.hidden_size, bias=False).to(model.device)
    poi_mapping_layer.requires_grad_(False)  # 不训练POI映射层

    # 初始化POI token的embedding
    initialize_poi_embeddings(model, tokenizer, poi_embeddings, poi_mapping_layer)
