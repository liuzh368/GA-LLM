...
class FusionGeoEncoder4096(nn.Module):
    """
    - geo_encoder 部分维度为 1024
    - 傅里叶部分维度为 128
    - 拼接后维度为 1152 => 映射到目标维度（4096）
    """
    def __init__(self,
                 gps_embed_dim=1024,      # geo_encoder 部分的维度
                 quadkey_length=25,
                 n=6,
                 vocab_size=4096,
                 fourier_output_dim=128, # 傅里叶部分的维度
                 final_dim=4096,         # 最终输出维度
                 dropout=0.1):
        super().__init__()
        self.gps_embed_dim = gps_embed_dim
        self.fourier_output_dim = fourier_output_dim
    ...
    def forward(self, latitude, longitude):
        ...
        token_embeddings = self.gps_embed_model(qk_idx_tensor)  # [1, seq_len, gps_embed_dim]
        emb_geo = self.gps_encoder(token_embeddings)            # [1, gps_embed_dim]

        # Step B: 傅里叶部分提取特征
        emb_fourier = self.fourier_encoder(latitude, longitude)  # [1, fourier_output_dim]

        # Step C: 拼接特征
        fused = torch.cat([emb_geo, emb_fourier], dim=-1)  # [1, gps_embed_dim + fourier_output_dim]

        # Step D: 映射到目标维度
        final_output = self.final_linear(fused)  # [1, final_dim]
        return final_output
