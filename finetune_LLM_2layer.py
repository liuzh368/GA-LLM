...
def initialize_poi_embeddings(poi_embedding_layer, poi_embeddings, device):
    """Initialize the POI embedding layer with raw POI embeddings."""
    with torch.no_grad():
...
def embedding_hook(module, inputs, output):
    """Embedding hook function to process POI token embeddings."""
    ...
    linear_mapping_layer = module.linear_mapping_layer
    ...
    if poi_token_ids.size(0) > 0:
        ...
        # 使用三层的 linear_mapping_layer 将其映射到 4096 维
        poi_token_embedding = linear_mapping_layer(original_poi_embeddings)
...
def apply_embedding_hook(model, poi_embedding_layer, poi_token_start_id, linear_mapping_layer):
    ...
    embedding_layer.linear_mapping_layer = linear_mapping_layer.to(embedding_layer.weight.device)
...
def train():
    ...
    # 初始化POI token的embedding，不应用线性映射层
