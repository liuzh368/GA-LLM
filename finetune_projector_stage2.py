...
class ModelArguments:
    ...
    pre_poi_mapping_layer: Optional[str] = field(default=None, metadata={
        "help": "Path to the pretrained linear mapping layer (pt file)."})
...
def embedding_hook(module, inputs, output):
    ...
    if poi_token_ids.size(0) > 0:
        ...
        print("Sample of linear_mapping_layer weights:", module.linear_mapping_layer.weight.data[0][:5])
...
def save_combined_linear_mapping_layer(model, output_path, alpha, r):
    # 合并 LoRA 权重到 linear_mapping_layer
    with torch.no_grad():
        for name, module in model.named_modules():
            if "linear_mapping_layer" in name and hasattr(module, "lora_A") and hasattr(module, "lora_B"):
                # 保存原始 linear_mapping_layer 的权重
                original_weight_path = os.path.join(output_path, "linear_mapping_layer_original.pt")
                original_state = {
                    "weight": module.weight.data.cpu(),
                    "bias": module.bias.data.cpu() if module.bias is not None else None
                }
                torch.save(original_state, original_weight_path)
                print(f"Original linear_mapping_layer weights saved to: {original_weight_path}")

                # 计算并合并 LoRA 权重
                lora_weight = (alpha / r) * (module.lora_B["default"].weight @ module.lora_A["default"].weight)
                combined_weight = module.weight.data + lora_weight
                combined_bias = module.bias.data if module.bias is not None else None

                # 保存合并后的权重
                combined_state = {
                    "weight": combined_weight.cpu(),
                    "bias": combined_bias.cpu() if combined_bias is not None else None
                }
...
def train():
    ...
    linear_mapping_layer = nn.Linear(128, model_embedding_dim).to(device)

    if model_args.pre_poi_mapping_layer is not None:
        try:
            # 加载预训练的 state_dict
            pretrained_state_dict = torch.load(model_args.pre_poi_mapping_layer, map_location=device)
            # 将调整后的 state_dict 加载到 `linear_mapping_layer`
            linear_mapping_layer.load_state_dict(pretrained_state_dict)
            print("Loaded pretrained linear_mapping_layer with adjusted state_dict.")
        except Exception as e:
            print(f"Failed to load pretrained linear_mapping_layer: {e}")
            raise

    # 将 linear_mapping_layer 添加到模型中作为一个属性
    model.linear_mapping_layer = linear_mapping_layer
    print("Initial linear_mapping_layer weights:", model.linear_mapping_layer.weight.data[0][:5])
    ...
    if training_args.low_rank_training:
        lora_config = LoraConfig(
            ...
            target_modules=["linear_mapping_layer"],  # 仅对 linear_mapping_layer 进行 LoRA 微调
            lora_dropout=0.1,
            bias="none",
            task_type="CAUSAL_LM",
        )
        # 确保 `get_peft_model` 已调用
    ...
    # linear_mapping_layer = nn.Linear(128, 4096)
    # linear_mapping_layer.weight.data = linear_mapping_layer_state["weight"]
    # if linear_mapping_layer_state["bias"] is not None:
    #     linear_mapping_layer.bias.data = linear_mapping_layer_state["bias"]
    #
    # # 将该 layer 赋值到新模型中
    # # model.linear_mapping_layer = linear_mapping_layer  # 根据需要使用
