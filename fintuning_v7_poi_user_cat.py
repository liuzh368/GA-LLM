...
class DataArguments:
    ...
    cat_embedding_path: str = field(default=None, metadata={"help": "Path to the Cat embeddings (.npy file)."})  # 添加cat embedding路径
...
def load_cat_embeddings(cat_embedding_path):
    """Load the Cat embeddings from a .npy file."""  # 新增函数，加载cat embeddings
    cat_embeddings = np.load(cat_embedding_path)
    return torch.tensor(cat_embeddings)
...
def initialize_cat_embeddings(model, tokenizer, cat_embeddings):
    """Directly initialize the Cat tokens embeddings without any mapping."""  # 新增函数，初始化cat embeddings
    embedding_layer = model.get_input_embeddings()
    embedding_weight = embedding_layer.weight.data

    # 将 cat embedding 转移到模型所在的设备
    cat_embeddings = cat_embeddings.to(model.device)

    # 将 cat token 转化为对应的 token id
    cat_token_ids = [tokenizer.convert_tokens_to_ids(f"<cat {i}>") for i in range(1, len(cat_embeddings) + 1)]

    # 确保 embedding 层的尺寸已经更新
    assert embedding_weight.size(0) >= len(cat_token_ids), "Embedding layer size mismatch"

    # 替换 cat token embedding
    for idx, cat_id in enumerate(cat_token_ids):
        embedding_weight[cat_id] = cat_embeddings[idx]

    print("Cat embeddings successfully initialized in the LLM's embedding layer.")

    # 释放 cat embeddings 的内存
    del cat_embeddings
    torch.cuda.empty_cache()  # 手动清理缓存的显存
...
def train():
    ...
    cat_embeddings = load_cat_embeddings(data_args.cat_embedding_path)  # 加载cat embedding
    ...
    cat_tokens = [f"<cat {i}>" for i in range(1, len(cat_embeddings) + 1)]  # 定义cat token
    special_tokens_dict = {"additional_special_tokens": poi_tokens + user_tokens + cat_tokens}
    ...
    # 初始化POI, User 和 Cat token的embedding，并释放内存
    with torch.no_grad():
        ...
        initialize_cat_embeddings(model, tokenizer, cat_embeddings)  # 初始化cat embedding
    ...
    print(f"Cat embeddings shape: {cat_embeddings.shape}")  # 输出cat embeddings形状
