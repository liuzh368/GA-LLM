...
def generate_qa_pairs(main_data, kqt=None, args=None):
    ...
    for user in tqdm(main_data['UserId'].unique()):
        ...
        for traj_id in user_data['pseudo_session_trajectory_id'].unique():
            ...
            num_traj = user_trajectory_data.shape[0]  # Fixed to use shape[0] for the number of rows
            if 'traj_id' in kqt.keys():
                ...
                user_historical_data = main_data[
                    (main_data['UserId'] == user) &
                    (main_data['UTCTimeOffsetEpoch'] < start_time_of_current_traj)
                ].tail(200 - num_traj)
...
def main():
    ...
    qa_pairs_train = generate_qa_pairs(train_data, kqt=kqt1, args=args)
    qa_pairs_test = generate_qa_pairs(test_data, kqt=kqt2, args=args)

    # Save the train QA pairs in JSON format
    qa_dict_train = [{"question": q, "answer": a} for q, a in qa_pairs_train]
    with open(f'{path}train_qa_pairs_kqt_simple.json', 'w') as json_file:
        json.dump(qa_dict_train, json_file)

    # Save the test QA pairs in TXT format
    with open(f'{path}test_qa_pairs_kqt_simple.txt', 'w') as txt_file:
