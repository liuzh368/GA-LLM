...
class DataArguments:
    ...
    projector_path: Optional[str] = field(default=None, metadata={"help": "Path to the pre-trained projector (linear mapping layer)."})  # 新增 projector_path
...
def train():
    ...
    if model_args.pre_poi_mapping_layer is not None:
        try:
            ...
            # 修改键名以匹配 `nn.Sequential` 的期望格式
            adjusted_state_dict = {}
            adjusted_state_dict["0.weight"] = pretrained_state_dict["layer_0_weight"]
            adjusted_state_dict["0.bias"] = pretrained_state_dict["layer_0_bias"]
            adjusted_state_dict["2.weight"] = pretrained_state_dict["layer_2_weight"]
            adjusted_state_dict["2.bias"] = pretrained_state_dict["layer_2_bias"]

            # 将调整后的 state_dict 加载到 `linear_mapping_layer`
            linear_mapping_layer.load_state_dict(adjusted_state_dict)
